...
Some assets come with additional attributes, health or other modifying values. Let’s keep track of those as well, specificially:

  - We need to be able to list all of these /special/ stats, separately from /health/, /iron/, and other progress values.
  - We should create them, but store their initial value at the very top of the org tree, like all other character stats
  - We could roll with them as both a modifier (like /wits/) or as a progress (like /supply/).

The following asset-related stat values need to be stored as =ironsworn-asset-something=, but displayed as =Something=, so we need functions to convert from a string to this symbol, and back:
...
Note that adding the asset-stat, =Dobby’s Health= will be converted to the stat, =asset-dobbys-health=, which will then be displayed (again after conversion) to the user as: =Dobbys Health=. But since most of these stats will be created from the Asset files (and not directly from the user), this shouldn’t be a problem.

Let’s make a few unit tests to verify this:
...
Use the [[help:rpgdm-ironsworn-store-default-character-state][rpgdm-ironsworn-store-default-character-state]] function to create and store an asset-related stat:
...
Now with the asset-stats available, we can /filter/ them from the =hash-table= returned from the [[help:rpgdm-ironsworn-current-character-state][rpgdm-ironsworn-current-character-state]] function, to produce an =alist= where the user-formatted name is joined with the asset keys.
...
And we can verify this with some unit tests:
...
With the ability to create an =alist= of all assets, we use this result to create a [[help:completing-read][completing-read]]-like function for selecting one of the stored asset-related stats.
...
Most asset-related stats, like a companion’s health, need to be adjusted, so using our =-choose= function as well as the =-adjust-stat= function, the [[help:rpgdm-ironsworn-asset-stat-adjust][rpgdm-ironsworn-asset-stat-adjust]] function is trivial:
...
Displaying the /value/ of an asset stat seems simple to write:
...
While we need to know the value of an asset stat, all of those stats should also show with the standard [[help:rpgdm-ironsworn-character-display][rpgdm-ironsworn-character-display]], as that function will call a function to get a string of all the asset-related stats and values (if any).
...
How would that actually look? Let’s write a unit test to make sure.
...
And now we need a few roll functions, for treating an asset-related stat as a modifier (for instance, the [[file:assets/paths/commander.org][Commander's Warband Strength]]):
